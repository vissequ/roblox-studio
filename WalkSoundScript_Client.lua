...
do
	...
	Sounds[SFX.WalkGrass] = Head:WaitForChild("WalkGrass")
	Sounds[SFX.WalkSand] = Head:WaitForChild("WalkSand")
	Sounds[SFX.WalkSnow] = Head:WaitForChild("WalkSnow")
	Sounds[SFX.WalkWoodPlank] = Head:WaitForChild("WalkWoodPlank")
	Sounds[SFX.WalkGround] = Head:WaitForChild("WalkGround")
	Sounds[SFX.WalkGrassLeafy] = Head:WaitForChild("WalkGrassLeafy")
	Sounds[SFX.WalkRock] = Head:WaitForChild("WalkRock")
	Sounds[SFX.WalkMud] = Head:WaitForChild("WalkMud")
	Sounds[SFX.WalkIce] = Head:WaitForChild("WalkIce")
	Sounds[SFX.WalkMarble] = Head:WaitForChild("WalkMarble")
...
do
	...
	local stateUpdateHandler = {
		...
		[Enum.HumanoidStateType.Running] = function(speed)
			if Humanoid.FloorMaterial == Enum.Material.Grass or Humanoid.FloorMaterial == Enum.Material.Fabric then
				
				sound = Sounds[SFX.WalkGrass]
				sound.PlaybackSpeed = 2.1 * (Humanoid.WalkSpeed/16)
			elseif Humanoid.FloorMaterial == Enum.Material.Snow then
				sound = Sounds[SFX.WalkSnow]
				sound.PlaybackSpeed = 2.1 * (Humanoid.WalkSpeed/16)
			elseif Humanoid.FloorMaterial == Enum.Material.Sand then
				sound = Sounds[SFX.WalkSand]
				sound.PlaybackSpeed = 1.8 * (Humanoid.WalkSpeed/16)
			elseif Humanoid.FloorMaterial == Enum.Material.WoodPlanks or Humanoid.FloorMaterial == Enum.Material.Wood then
				sound = Sounds[SFX.WalkWoodPlank]
				sound.PlaybackSpeed = 2 * (Humanoid.WalkSpeed/16)
			elseif Humanoid.FloorMaterial == Enum.Material.Ground or Humanoid.FloorMaterial == Enum.Material.Mud then
				sound = Sounds[SFX.WalkGround]
				sound.PlaybackSpeed = 2.2 * (Humanoid.WalkSpeed/16)
			elseif Humanoid.FloorMaterial == Enum.Material.LeafyGrass then
				sound = Sounds[SFX.WalkGrass]
				sound.PlaybackSpeed = 2.1 * (Humanoid.WalkSpeed/16)
						--right now same as grass
			elseif Humanoid.FloorMaterial == Enum.Material.Sandstone then
				sound = Sounds[SFX.WalkGround]
				sound.PlaybackSpeed = 2.2 * (Humanoid.WalkSpeed/16)
			elseif Humanoid.FloorMaterial == Enum.Material.Rock or Humanoid.FloorMaterial == Enum.Material.Granite or Humanoid.FloorMaterial == Enum.Material.Slate or Humanoid.FloorMaterial == Enum.Material.Brick or Humanoid.FloorMaterial == Enum.Material.Concrete then
				sound = Sounds[SFX.WalkMarble]
				sound.PlaybackSpeed = 2 * (Humanoid.WalkSpeed/16)
--				sound = Sounds[SFX.WalkRock]
--			elseif Humanoid.FloorMaterial == Enum.Material.Mud then
--				sound = Sounds[SFX.WalkMud]
			elseif Humanoid.FloorMaterial == Enum.Material.Ice then
				sound = Sounds[SFX.WalkIce]
				sound.PlaybackSpeed = 2.2 * (Humanoid.WalkSpeed/16)
			elseif Humanoid.FloorMaterial == Enum.Material.Marble or Humanoid.FloorMaterial == Enum.Material.Cobblestone then
				sound = Sounds[SFX.WalkMarble]
				sound.PlaybackSpeed = 2 * (Humanoid.WalkSpeed/16)
			else
				--i think this is the reason for the phantom walking sound at times
		...
		if useUpdatedLocalSoundFlag then threshold = speed else threshold = Util.VerticalSpeed(Head) end
			if activeState ~= Enum.HumanoidStateType.Swimming and threshold > 1.1 then
										--threshold was 0.1 but I changed it to 1.1 so it didn't spam the splashes when swimming near sea level
		...
		[Enum.HumanoidStateType.Jumping] = function()
			...
			stopPlayingLoopedSoundsExcept()
			if JumpSoundCount == 0 then
				sound = Sounds[SFX.Jumping]
				Util.Play(sound)
				JumpSoundCount = JumpSoundCount + 1
			else
				if JumpSoundCount < 4 then
					JumpSoundCount = JumpSoundCount + 1
				elseif JumpSoundCount == 4 then
					JumpSoundCount = 0
				end
			end
		end;
